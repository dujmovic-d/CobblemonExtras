    ...
    private int execute(CommandContext<ServerCommandSource> ctx) {
        if (ctx.getSource().getPlayer() != null) {
            ...
            if (opponentExistingChallenge != null && !opponentExistingChallenge.isExpired() && opponentExistingChallenge.getChallengedPlayerUUID() == player.getUuid()) {
                ...
                BattleStartResult result = BattleBuilder.INSTANCE.pvp1v1(player, battlePartner, null,
                        null, BattleFormat.Companion.getGEN_9_SINGLES(),false,false, (serverPlayerEntity) -> Cobblemon.INSTANCE.getStorage().getParty(serverPlayerEntity));
                ...
                BattleRegistry.BattleChallenge challenge = new BattleRegistry.BattleChallenge(player.getUuid(),battlePartner.getUuid(), Objects.requireNonNull(Cobblemon.INSTANCE.getStorage().getParty(player).get(0)).getUuid(), 60);
